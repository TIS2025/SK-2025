...
public class ParthTeleop extends LinearOpMode {
...
//            mecanumDrive.FieldCentricDrive(gamepad1, gamepad1.right_trigger >= 0.5, false);
            ...
            if (gamepad1.a && !downFlag) {
                ...
                mecanumDrive.setDrivePowers(new PoseVelocity2d(new Vector2d(drive/3,strafe/3),turn/3));
            }
            //Todo: Bucket drop
            if (gamepad1.b)
            {
...
//                new DropHeightSequence(intake, outtake, hanger);
                runningActions.add(DropHeightSequence.getDropHeightAction(intake, outtake, hanger));
                 downFlag=true;
                mecanumDrive.setDrivePowers(new PoseVelocity2d(new Vector2d(drive/3,strafe/3),turn/3));
...
//            }
            ...
            telemetry.addData("Lifter Position", robot.lifter2.getCurrentPosition());
            ...
            telemetry.addData("lifter Current:",robot.lifter.getCurrent(CurrentUnit.AMPS));
            telemetry.addData("lifter2 Current:",robot.lifter2.getCurrent(CurrentUnit.AMPS));

            telemetry.addData(" Hang current", robot.leftHang.getCurrent(CurrentUnit.AMPS));
            telemetry.update();
        }
    }
